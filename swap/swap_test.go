...
func TestRestoreBalanceFromStateStore(t *testing.T) {
	...
	// create a test swap account
	swap, testDir, logDir := newBaseTestSwap(t, ownerKey, testBackend)
...
func newBaseTestSwap(t *testing.T, key *ecdsa.PrivateKey, backend *swapTestBackend) (*Swap, string, string) {
	t.Helper()
...
func newTestSwap(t *testing.T, key *ecdsa.PrivateKey, backend *swapTestBackend) (*Swap, func()) {
	t.Helper()
	swap, dir, logDir := newBaseTestSwap(t, key, backend)
	clean := func() {
		swap.Close()
		os.RemoveAll(dir)
		os.RemoveAll(logDir)
